...
export type IconName = "panelLeft" | "panelLeftClose" | "house" | "dumbbell" | "x" | "database"
  ...
  "logout" | "gear" | "user" | "login" | "stickynote" | "google" | "github" | "twitter" | 'image' | 'volume2' | 'mic' | 'micOff' | 'video' |
  ...
  return (
    <Switch fallback={<Show when={import.meta.env.DEV}><p>Icon not found: {local.name}</p></Show>}>
      ...
      <Match when={local.name === "github"}>
        <GithubIcon width={defaultWidth} height={defaultHeight} class={defaultClass} {...others} />
      </Match>
      <Match when={local.name === "twitter"}>
        <TwitterIcon width={defaultWidth} height={defaultHeight} class={defaultClass} {...others} />
      </Match>
